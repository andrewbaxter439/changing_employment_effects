...
#' First, run 'R/install_and_setup.R'
...
sample_hours_wages <- employed_pop |>
  ...
  ## ethnicity_collapsed, caring responsibilities, industry
    .by = c(age_collapsed, dgn, deh_c3, dms_collapsed, dhr, ethnicity_collapsed, lcr01, lindi)
  ) |>
...
replacement_workers <- not_in_employment_pop |>
  ...
  left_join(
    sample_hours_wages,
    by = join_by(age_collapsed, dgn, deh_c3, dms_collapsed, dhr, ethnicity_collapsed, lcr01)
    ) |> 
  replace_na(list(n = 1)) |> 
...
lhw_mod <- employed_pop |> feols(
  lhw ~ age_collapsed + dgn + deh_c3 +  n_ch +
...
yem_mod <- employed_pop |> feols(
  yem ~ age_collapsed + dgn + deh_c3 + n_ch +
    dms_collapsed + dhr + ethnicity_collapsed + lcr01 + ddi03 + i(drgn1) +
    lhw,
  data = _
)
...
yds_mod <- employed_pop |> feols(
  yds ~ age_collapsed + dgn + deh_c3 +  n_ch +
    dms_collapsed + dhr + ethnicity_collapsed + lcr01 + ddi03 + i(drgn1) +
    lhw + yem,
  data = _
)
...
bch_mod <- employed_pop |> feols(
  bch ~ age_collapsed + dgn + deh_c3 + n_ch + 
    dms_collapsed + dhr + ethnicity_collapsed + lcr01 + ddi03 + i(drgn1) +
    lhw + yem + yds,
  data = _
)
...
bsa_mod <- employed_pop |> feols(
  bsa ~ age_collapsed + dgn + deh_c3 +  n_ch + 
    dms_collapsed + dhr + ethnicity_collapsed + lcr01 + ddi03 + i(drgn1) +
    lhw + yem + yds,
  data = _
)
...
bho_mod <- employed_pop |> feols(
  bho ~ age_collapsed + dgn + deh_c3 +  n_ch +
    dms_collapsed + dhr + ethnicity_collapsed + lcr01 + ddi03 + i(drgn1) +
    lhw + yem + yds,
  data = _
)
...
new_vals <- get_matches(matched_pop_u25) |> 
  bind_rows(get_matches(matched_pop_o25)) |> 
