...
make_pops_from_matches <- function(filename = "data/UK_2023_b1.txt", seed = 150, cli_args = NULL) {
  ...
  raw_data <- readr::read_tsv(filename)
  ...
  if (!is.null(cli_args)) seed_addin <- glue::glue("_{seed}")
...
#   }
#   
#   make_pops_from_matches(seed = seed, cli_args = args)
# }